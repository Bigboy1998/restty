...
test("overlay scrollbar layout uses fixed thickness and dynamic thumb sizing", () => {
  ...
  const bodyMatch = source.match(
    /function computeOverlayScrollbarLayout\([\s\S]*?\): OverlayScrollbarLayout \| null \{[\s\S]*?return \{ total, offset, len, denom, width, trackX, trackY, trackH, thumbY, thumbH \};\n  \}/,
  );
  ...
  expect(body.includes("canvas.height")).toBe(true);

...
const entry = resolve("reference/text-shaper/src/index.ts");
const outDir = resolve("playground/public");
const outFile = resolve(outDir, "text-shaper.js");
...
if (mapOutput) {
  if (mapOutput.path !== outMap) {
    await rm(outMap, { force: true });
    await rename(mapOutput.path, outMap);
  }
}

console.log(`text-shaper bundle: ${outFile}`);

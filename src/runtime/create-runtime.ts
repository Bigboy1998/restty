...
import type { ResttyFontSource, ResttyApp, ResttyAppOptions, ResttyShaderStage } from "./types";
...
export function createResttyApp(options: ResttyAppOptions): ResttyApp {
  ...
  function normalizeSelectionCell(cell) {
    ...
    return normalizeGridSelectionCell(cell, rows, cols, lastRenderState?.wide);
  }
  ...
  function ensureWebGLStageTargets(state: WebGLState): WebGLStageTargets | null {
    ...
    webglStageTargets = createWebGLStageTargets(gl, canvas.width, canvas.height);
    return webglStageTargets;
  }
  ...
  function ensureWebGPUStageTargets(state: WebGPUState): WebGPUStageTargets | null {
    ...
    webgpuStageTargets = createWebGPUStageTargets(
      state.device,
      state.format,
      canvas.width,
      canvas.height,
  ...
  function selectionForRow(row, cols) {
    return selectionRangeForRow(selectionState, row, cols);
  }
  ...
  function getSelectionText() {
    ...
    return extractSelectionText(selectionState, rows, cols, (idx) =>
  ...
  function tickWebGL(state: WebGLState) {
    ...
    const stageTargets =
      compiledWebGLShaderStages.length > 0 ? ensureWebGLStageTargets(state) : null;
